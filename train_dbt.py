...
def train(train_loader, model, loss_fcn, g_lam, optimizer, args, num_avg_iter=500):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        print("loss {} group loss {}".format(loss, loss_sg))
        run_loss += loss.item()

        # print("i {} t loss {}".format(i, run_loss))
        if (i % num_avg_iter == 0) and i > 0:
            print(
                "Training loss batch {} running avg {}".format(
                    i, float(run_loss) / num_avg_iter
                )
            )
            run_loss = 0
...
def main(args):
    ...
    if not args.eval_only:
        ...
        if train_dataset.class_to_idx != test_dataset.class_to_idx:
            raise Exception("train class to index {} != test {}".format(train_dataset.class_to_idx, test_dataset.class_to_idx))
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            recall = validate(test_loader, model, criterion, args.g_lambda, epoch, args)
            scheduler.step()
            last_epoch = epoch
...
parser.add_argument("--la", default=1.0e-04, type=float, help="lambda")
parser.add_argument(
...
parser.add_argument("--pretrain-C", default=None, type=int, help="Number of classes from pre-training")
parser.add_argument(
...
parser.add_argument("-M", "--use-cutmix-or-mixup", action="store_true", help="use CutMix or Mixup")
