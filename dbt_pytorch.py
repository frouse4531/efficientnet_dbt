...
class GroupConv(nn.Module):
    ...
    def forward(self, x):
        ...
        self.loss = loss_single / ((channels / 512.0) * (channels / 512.0)) # loss_single.repeat(b)
...
def lr_scheduler(
    optimizer, lr, lr_gamma, decay_epoch, max_epoch, name="exp", lr_min=None
):
    """
    ...
    name: name of scheduler to use.  Allowed names
        exp - ExponentialLR
        step - StepLR
        cos - add a cosine annealing scheduler.  We reduce the lr by step scheduler before we apply cos.
    ...
    if name == "exp":
        ...
        decay_epoch = max(
            int(decay_epoch), 1
        )  # make sure decay_epoch is an integer > 0
        scheduler_1 = StepLR(optimizer, decay_epoch, gamma=gamma)
        if name == "cos":
            eta_min = 0 if lr_min is None else lr_min
            scheduler_2 = CosineAnnealingLR(optimizer, decay_epoch, eta_min=eta_min)
            scheduler = ChainedScheduler([scheduler_1, scheduler_2], optimizer)
        else:
